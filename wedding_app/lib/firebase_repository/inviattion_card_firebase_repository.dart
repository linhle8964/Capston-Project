...
class FirebaseInvitationCardRepository implements InvitationCardRepository {
  ...
  Stream<List<InvitationCard>> getAllInvitationCard(String weddingID) {
    return FirebaseFirestore.instance
        .collection('wedding')
        .doc(weddingID)
        .collection('invitation_card')
        .snapshots()
        .map((snapshot) {
      return snapshot.docs
          .map((doc) =>
              InvitationCard.fromEntity(InvitationCardEntity.fromSnapshot(doc)))
          .toList();
    });
  }

  @override
  Future<void> addNewInvitationCard(
      InvitationCard invitationCard, String weddingID) {
    return FirebaseFirestore.instance
        .collection('wedding')
        .doc(weddingID)
        .collection('invitation_card')
